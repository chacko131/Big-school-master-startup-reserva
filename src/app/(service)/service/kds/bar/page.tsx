/**
 * Archivo: page.tsx
 * Responsabilidad: KDS de barra — cola de ítems BAR en estado QUEUED o PREPARING.
 * Tipo: UI (server)
 */
...
export default async function BarKdsPage() {
  await requireServiceRole(["STAFF_BAR", "RESTAURANT_OWNER", "MANAGER"]);

  const result = await fetchKdsQueueAction("BAR");
    ...
    return (
      <div className="flex-1 flex flex-col bg-black min-h-0 items-center justify-center gap-3">
        ...
        <p className="text-sm font-semibold text-red-400">
          <T>Error al cargar la cola de barra</T>: {result.error}
        </p>
      </div>
    );
  }
